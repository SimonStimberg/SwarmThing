...
void checkDepth() {
  ...
  float newDepth = height * constrain(map(flock.boids.size(), 0, 500, initDepth, 0.0), 0.0, initDepth);
  depth = lerp(depth, newDepth, 0.05);
  wall.adjustWall(oldDepth - depth);
  avoider.adjustWall(oldDepth - depth);
  //depth = newDepth;
...
void newWall() {
  ...
  avoider.addPoint(width*0.377-offset*2,0+depth+75);
  avoider.addPoint(width*0.377,0+depth+75+offset*2);
  avoider.addPoint(width-width*0.377,0+depth+75+offset*2);
  avoider.addPoint(width-width*0.377+offset*2,0+depth+75);
  avoider.addPoint(width-width*0.377+offset*2,0);
  avoider.addPoint(width*0.377-offset*2,0);
