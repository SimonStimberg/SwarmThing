...
class Boid {
  ...
  Boid() {
    ...
    // which gate to spawn
    if (int(random(2)) < 1) {
      x = matteLeft+outerColumnWidth+smallGateWidth+innerColumnWidth;      
      velX = random(-1, -0.2);
    } else {
      x = matteLeft+outerColumnWidth+smallGateWidth+innerColumnWidth+bigGateWidth;
      velX = random(0.2, 1);
    }
    ...
    r = random(2.5, 5.0);  // size of the boid
    maxspeed = random(2.0, 3.0);  // max speed a boid can move
    maxforce = 0.05;
    wandertheta = 0;
  }
  ...
  void behavior(ArrayList<Boid> boids) {
    ...
    if(!die) {
      ...
      int offPos;
      if (position.x < width*0.5) offPos = -100; else offPos = width + 100;
      PVector beatIt = seek(new PVector(offPos, height-200));
      beatIt.mult(2.0);
      applyForce(beatIt);
    }
