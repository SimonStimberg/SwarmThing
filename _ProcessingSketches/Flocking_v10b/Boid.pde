...
class Boid {
  ...
  PVector getFood() {
    ...
    float sensationRadius = 300;
    PVector closestFood = null;
    float closestDist = 1000000;
    int foodIndex = -1;
    
    //for (PVector s : food.stuff) {
    for (int i = 0; i < food.stuff.size(); i++) {
      PVector s = food.stuff.get(i);
      PVector distance = PVector.sub(s,position);
    ...
    //PVector foodPosition = new PVector(mouseX, mouseY);
    
    //PVector distance = PVector.sub(foodPosition,position);
    
    if(closestDist < sensationRadius) {   
      
      if(closestDist < 5) {
        food.eaten(foodIndex);
      }
