...
class Boid {
  ...
  void render() {
    ...
    beginShape();
    vertex(0, -r*2);
    bezierVertex(r*2, -r*2, r, r, 0, r*2);
    bezierVertex(-r, r, -r*2, -r*2, 0, -r*2);
    endShape();
    popMatrix();
  }
  ...
  PVector boundaries(Wall p) {
    ...
    if (worldRecord < p.radius) {
        ...
        float theta = PVector.angleBetween(direction, velocity);
        //println(theta);
        //if(theta > HALF_PI) {
        //  theta = HALF_PI - theta;
        //}
        // calculate the reflection used as desired target to steer away from the wall
        // for this use trigonomy
        float gk = sin(theta) * predict.mag();
        
        strokeWeight(1);
        stroke(0);
        
        target = predictpos.copy();
        
        //fill(0,0,255);
        //ellipse(target.x, target.y, 10, 10);
        
        PVector pointer = PVector.sub(target, normal);
        pointer.normalize();
        pointer.mult(gk*2);
        target.add(pointer);
        //target.rotate(-theta*2);
        ////target.add(desired);
        ...
        //target = PVector.sub(target, velocity);
