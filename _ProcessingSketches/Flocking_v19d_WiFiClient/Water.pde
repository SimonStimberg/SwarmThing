...
class Ground {
  ...
  void update() {
    ...
    colorMode(HSB, 360, 100, 100);
    int hue1 = int(map(pollution, 0, 1.0, 200, 168));
    c1 = color(hue1, 100, 100);
    int hue2 = int(map(pollution, 0, 1.0, 30, 11));
    c2 = color(hue2, 100, 100);
    //colorMode(RGB);
    ...
    for (int i = 0; i < cols; i++) {
      for (int j = 0; j < rows; j++) {
        ...
        colorMode(HSB, 360, 100, 100);
        float h = hue(inter);        
        float s = saturation(inter) * 1.5;
        float b = brightness(inter) * 2;
        inter = color(h, s, b);

        //colorMode(HSB, 360, 100, 100);
        //color inter = color(map(intensity, 0.0, 1.0, 250, 120), 100, 100);
        //colorMode(RGB);
        noStroke();
        fill(inter);
